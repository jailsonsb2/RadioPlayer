...
//API URL /
const API_URL = 'http://localhost:8000/radio_info/?radio_url='+URL_STREAMING
...
async function getStreamingData() {
    try {
      ...
      const streamTitleUrl = `${baseUrl.replace('/radio_info/', '/get_stream_title/')}?url=${encodeURIComponent(URL_STREAMING)}`;
  
      const streamTitleResponse = await fetch(streamTitleUrl);
  
      if (!streamTitleResponse.ok) {
        throw new Error(`Erro na requisição /get_stream_title/: ${streamTitleResponse.status} ${streamTitleResponse.statusText}`);
      }
      ...
      if (data.last_played) {
        ...
        const currentSong = data.last_played.song ? data.last_played.song.replace(/'/g, '\'').replace(/&/g, '&') : 'Nome da Música'; 
        const currentArtist = data.last_played.artist ? data.last_played.artist.replace(/'/g, '\'').replace(/&/g, '&') : 'Nome do Artista'; 
        ...
        if (lastSong !== data.last_played.song) {
          // Utiliza data.history para o histórico
          for (let i = 0; i < data.history.length && i < 5; i++) { 
            page.refreshHistoric(data.history[i], i);
          }
  
          // Atualiza a última música
          lastSong = data.last_played.song;
        }
      } else {
        console.warn("Dados da última música não encontrados na resposta da API.");
      }
